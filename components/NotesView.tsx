...
const NotesView: React.FC<NotesViewProps> = ({ note, onSave, onStartChat, onBack, showToast, shouldAutoPlay, onStatsUpdate }) => {
  ...
  const handleStopRecording = useCallback(() => {
    // Always update UI state first to prevent "stuck" button
    setIsRecording(false);

    if (!mediaRecorderRef.current || mediaRecorderRef.current.state === 'inactive') {
      console.warn("Stop called but recorder is inactive or missing");
      return;
    }
    ...
    try {
      ...
      console.error("Error stopping recorder logic:", err);
      setIsRecording(false);
    }
